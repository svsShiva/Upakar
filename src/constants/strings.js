export const screens = {
    DASHBOARD: "Dashboard"
}

export const dashboardStrings = {
    LOADING_MESSAGE: "Loading ..."
}

export const loginStrings = {
    WELCOME_MESSAGE: "Upakar Welcomes you!"
}

export const dashboardDetailStrings = {
    WELCOME_MESSAGE: "Create Help"
}

export const myHelpsStrings = {
    WELCOME_MESSAGE: "my Helps",
    LOADING_MESSAGE: "Loading ..."
}

export const myHelpsDetailStrings = {
    WELCOME_MESSAGE: "my help detail"
}

export const placedBidsStrings = {
    WELCOME_MESSAGE: "Placed bids",
    CREDTS:'Credits',
    DURATION:'Duration',
    END_TIME: "End's In",
    
}

export const receivedBidsStrings = {
    WELCOME_MESSAGE: "Received Bids"
}

export const createHelpStrings = {
    WELCOME_MESSAGE: "Create Help"
}
export const placedbidDetailStrings= {
    WELCOME_MESSAGE:'Placed Bid Details'
}
export const receivedbidDetailStrings={
    WELCOME_MESSAGE:'Received Bid Details',
    ACCEPT_BID: 'Accept Bid'
}